 ...
 * A Facebook Provider
 */
class WistFacebookProvider(routesService: RoutesService,
  ...
  override val id = WistFacebookProvider.Facebook
    ...
    client.retrieveProfile(MeApi + accessToken).map { me =>
      (me \ Error).asOpt[JsObject] match {
        ...
        case _ =>
          ...
          AuthorizedProfile(id, email.getOrElse("No Email"), name, lastName, authMethod, avatarUrl, oAuth2Info = Some(info))
      }
    } recover {
