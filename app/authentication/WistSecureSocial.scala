 ...
 *
 */
trait WistSecureSocial[U] extends Controller {
  implicit val env: WistRuntimeEnvironment[U]
  ...
  protected def notAuthorizedPage()(implicit request: RequestHeader): Html =  Html("securesocial.views.html.notAuthorized()")
    ...
    Future.successful {
      render {
        ...
        case Accepts.Html() => Redirect(env.routes.loginPageUrl).
          ...
          .withSession(request.session + (WistSecureSocial.OriginalUrlKey -> request.uri))
...
object WistSecureSocial {
   ...
   */
  def currentUser[U](implicit request: RequestHeader, env: WistRuntimeEnvironment[U], ec: ExecutionContext): Future[Option[U]] = {
